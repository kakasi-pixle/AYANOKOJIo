...
await conn.sendMessage(m.chat, { react: { text: '🥺', key: m.key } })
 ...
     conn.reply(m.chat, `*عمي هو مايكي و كاكاشي طبعا 🫶🏻💗*`, m) //wm, null, [['Menu', '#menu']], m) botones :V
}
    if (/^بوت$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat, `*اسمي ايانوكوجي يا ورع 🗿*`, m) //wm, null, [['Menu', '#menu']], m) botones :V
await conn.sendMessage(m.chat, { react: { text: '🗿', key: m.key } })
}
...
await conn.sendMessage(m.chat, { react: { text: '🗿', key: m.key } })
   ...
   if (/^بويكا$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat,'*قلب ايانوكوجي 🐦💗*',m) //wm, null, [['Menu', '#menu']], m) botones :v
await conn.sendMessage(m.chat, { react: { text: '💗', key: m.key } })
   }
 ...
     conn.reply(m.chat, `*امشي يابت غوري مش ناقصة تلزيق 🗿*`, m) //wm, null, [['Menu', '#menu']], m) botones :V
   ...
   if (/^مايكي $/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat,'*عمو مايكي و كاكاشي 🦦*', m) //wm, null, [['Menu', '#menu']], m) botones :V
...
    conn.reply(m.chat, `*الحمد لله و انت 🐧*`, m) //wm, null, [['Menu', '#menu']], m) botones :V 
 ...
 if (/^كاكاشي$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat, `*صديق مايكي الروح بل الروح 🐧⚡*`, m) //wm, null, [['Menu', '#menu']], m) botones :V 
